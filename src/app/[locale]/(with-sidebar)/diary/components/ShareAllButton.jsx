...
exports.default = ShareAllButton;
...
function ShareAllButton(_a) {
    var entries = _a.entries, locale = _a.locale;
    ...
    function getContent() {
        ...
        return {
            title: "".concat((0, date_fns_1.format)(firstEntry.date, "MMMM d, yyyy"), " - ").concat((0, date_fns_1.format)(lastEntry.date, "MMMM d, yyyy")),
    ...
    return (<>
			<button type="button" onClick={handleCopy} className="p-2 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-all cursor-pointer" title={t("diary.copyEntry")} aria-label={t("diary.copyEntry")}>
				{isCopied ? (<lucide_react_1.Check className="h-4 w-4"/>) : (<lucide_react_1.Copy className="h-4 w-4"/>)}
			</button>
			<button type="button" onClick={handleShare} className="p-2 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-all cursor-pointer" title={t("diary.shareAllEntries")} aria-label={t("diary.shareAllEntries")}>
				<lucide_react_1.Share2 className="h-4 w-4"/>
			</button>
			<div ref={contentRef} hidden>
				{entries.map((entry, index) => (<div key={entry.id}>
						<div className="whitespace-pre-line">
							{(0, date_fns_1.format)(new Date(entry.date), "MMMM d, yyyy")}
							{"\n\n"}
